...
var column  = document.querySelector('.col-6');
...
function displayButton(event){
    ...
    for (i=1; i< 6; i++){
        document.getElementById(`button${i}`).addEventListener('click',selectTopic )
    }
    function selectTopic () {
    document.querySelector('.col-6').style.display = "block"
    }
...
function getNextQuestion(questionNumber = 0) {
    ...
    for (var i = 0; i < answers.length; i++) {
        ...
        questionBox.innerHTML += `<button id='${i}' onClick="checkAnswer(event,'${answer}','${i}')" class="btn btn-info" style="margin:5px;width: 500px;border-radius:15px">${answer}</button></br>`;
    }
}
...
function checkAnswer(event, answer, correctId) {
    ...
    if (questions[questionNumber].correct === answer) {
        ...
        wrongAnswer()
        document.querySelector('.back').innerHTML = `<div class="new face back"> <h2 id='explanatioN'> ${questions[questionNumber].explanation} </h2> 
        <button type="button" style="width: 150px;" onClick="nextquestion(event)" id="nextQ" class="btn btn-primary ">Next Question </button> </br>  </div> `
        questionNumber += 1;         
    }
}
...
function andQuestion () {
    document.querySelector('#card-back').style.display = "none"
    document.querySelector('#questionBox').style.display = "none"
    document.querySelector("#col-6id").innerHTML = `<button type="button" style="width: 150px;" onClick='index.html' class="btn btn-primary ">Finish </button> </br>  </div> `
}
